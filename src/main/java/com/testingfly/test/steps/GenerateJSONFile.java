 ...
 * Processed chained request data
...
public class GenerateJSONFile extends APITestBase 
{
	...
	public void createJSONFile(Map<String, String> params) throws FilloException, IOException
	{
		...
		if(!jsonFile.exists()){
			System.err.println("\nERROR loading json payload template. Please verify that the file exist.");
		}
		...
		if (jsonFile.isFile() && !templateDefaulted ){
				...
				//System.out.println("Debug Payload*** "+jsonNode.toString());
		...
		if(requestType.equalsIgnoreCase("POST") || requestType.equalsIgnoreCase("PATCH") || requestType.equalsIgnoreCase("PUT")){
			...
			String fileName = params.get("test suite id")+"_"+params.get("test case id");//params.get("RequestJSON"); String tc_id = 
			...
			if(templateDefaulted){
				...
				//FileUtils.writeStringToFile(newHtmlFileInProjectFolderOfSummary,RestClient.prettyPrint(((StringEscapeUtils.unescapeJava(payload))), false),Charset.defaultCharset(), false);
		...
		if(params != null && !params.isEmpty())
		  {
			  if (jsonNode.isObject()) {
			      ...
			      while (iter.hasNext()) {
			        ...
			        String key = null;
			        //System.out.println("JSON path: $" +pathPrefix+entry.getKey()+"/"+ params.get("$"+pathPrefix+entry.getKey()));
			        ...
			        else
			        {
			        	setJsonFieldValues(pathPrefix + entry.getKey()+"$",entry.getValue(), params);
			    				...
			    				try{
				    				...
				    				//System.out.println("Field Name: "+"$"+fieldName.substring(0, fieldName.length()-1));
				    				String arrayStr[] = params.get(fieldName).split(",");
				    				//System.out.println("Guest List: "+Arrays.toString(arrayStr));
			    					...
			    					System.err.println("***WARNING: Invalid entry for "+fieldName+". Using default value from template.***");
									      ...
									      for (int i = 0; i < arrayNode.size(); i++) {
									    	  ...
									    	  //setJsonFieldValues(currentPath , arrayNode.get(i), params);	
									      }
						    }			  
		...
		if(jsonObject != null )
    	{
			...
			if (jsonObject.equals("") )
    		{
    		...
    		} else if (jsonObject.contains("DEFAULT"))
	    		{
	    			...
	    			//System.out.println("Debug: "+String.valueOf(GlobalHashMap.getValue(jsonObject)));
	    		}
	    		//else if(!jsonObject.equals("$SKIP$"))
	    		else if(!jsonObject.equals(""))
	    		{
	    			//System.out.println("Data: "+entry.getKey()+"/"+jsonObject);
	    			try {
    				...
    				if(jsonObject.contains("EMPTY")){
    					...
    					//objectNode.put(entry.getKey(),jsonObject.trim());
	...
	public void getChainedRequestDataHeader(Map<String, String> params) throws IOException{
		...
		//System.out.println("Chained Data: "+Arrays.toString(xmlString));
		for (String key:KeySet){
			if (params.get(key).contains("#header") && !key.contains("#")){
				//String field = params.get(key).split(":")[1].trim();
				...
				//System.out.println("Field: "+field+"**fileName:"+fileName);
				String val = readFile(fileName, field);//get data from file
				
				//System.out.println("***Val: "+key+":"+temp+val);				
				params.put(key, temp+val);
				//System.out.println("Updated Chained Keyset** "+key+":"+params.get(key));
			}
		}
	...
	public void getChainedRequestDataBody(Map<String, String> params) throws IOException{
		...
		for (String key:KeySet){
			...
			if (params.get(key).contains("#body") && !key.contains("#")){
				String temp=params.get(key).split("#")[0]+"#";
				String field = params.get(key).replaceAll(".*#", "#").split(":")[1].trim();
