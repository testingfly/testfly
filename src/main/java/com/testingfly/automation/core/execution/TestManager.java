...
import com.testingfly.automation.core.data.ITestInstance;
import com.testingfly.automation.core.data.ITestSet;
import com.testingfly.automation.core.data.TestInstance;
import com.testingfly.automation.core.data.TestSet;
import com.testingfly.automation.core.enums.DataInputType;
import com.testingfly.automation.core.enums.TestFilterType;
import com.testingfly.automation.core.listeners.CustomLogger;
import com.testingfly.automation.core.reporting.TestReportComparator;
import com.testingfly.automation.core.swagger.SwaggerData;
import com.testingfly.automation.core.table.Record;
import com.testingfly.automation.core.table.Table;
import com.testingfly.automation.core.testngdata.TestNGSuite;
import com.testingfly.automation.core.utils.Excel;
import com.testingfly.automation.core.utils.TestUtils;
import com.testingfly.automation.core.utils.Utils;
import com.testingfly.automation.core.utils.XMLGenerator;
...
public class TestManager implements ITestManager {
	...
	//public final ALMAttachTarget attachTarget = Configurables.almAttachTarget.toLowerCase().contains("run") ? ALMAttachTarget.RUN : ALMAttachTarget.TEST;
	...
	public void execute() throws Exception{
		...
		Utils.pdfFileName = "API_Test_Result_"+Config.getProp("Environment")+"_"+Config.getProp("tagName")+"_"+ts+".pdf";
		...
		List<String> listeners = new ArrayList<String>(Arrays.asList(
				"com.javaexcel.automation.core.reporting.HTMLReporter",
		...
		if (Configurables.dataInputType != DataInputType.XML){
			...
			if (jobName.equals("local"))
			{
				System.out.println("\n\nFor details, review the Test Report available at "+Configurables.uploadDirectory+"/index.html\n");
				System.out.println("Link to PDF report: "+"/reports/pdf/"+Utils.pdfFileName);

			}
			else{
				System.out.println("\n\nFor details, review the HTML Test Report available at "+ jenkinsURL +"/job/" + jobName +"/ws/"+Configurables.uploadDirectory+"/index.html\n");
				System.out.println("Link to PDF report: "+jenkinsURL + "/job/" + jobName+ "/ws/reports/pdf/"+Utils.pdfFileName);
	...
	/**
	 * Creates a summary report of all the Test cases executed with pass n fail status. This Summary report will be included in the Email body as part of the email notification post a jenkins daily execution job completion
	 */
	public void createSummaryReport()
	{
		String strSummaryFileContent="<!DOCTYPE HTML><head><meta http-equiv=\"Content-Type\" content=\"text/html; charset=UTF-8\"><title>Test Case Name</title></head><body><table width=\"80%\" border=\"2\" align=\"center\">"
				+ "<tr><td colspan=\"2\" align=\"left\"><b>Info: "+Config.getProp("URL")+"</b></td><td colspan=\"1\" align=\"left\"><b>BrowserName: "+Config.getProp("Browser")+"</b></td></td><td colspan=\"1\" align=\"left\"><b>"+(new SimpleDateFormat("MM/dd/yyyy").format(new Date(System.currentTimeMillis())))+"</b></td></tr>"
		...
		while(mapIterator.hasNext())
		{
			...
			if(testCaseMap.get(pair.getKey()).getStatus().toLowerCase().equals("passed")){intPassCounter++;strFonColor="green";}
			if(testCaseMap.get(pair.getKey()).getStatus().toLowerCase().equals("failed")){intFailCounter++;strFonColor="red";}
			
			strSummaryFileContent=strSummaryFileContent+"<tr><td align=\"center\" width=\"10%\" bgcolor=\"#deb887\"><b>Test "+intTestCounter+"</b></td><td align=\"left\" width=\"40%\" bgcolor=\"#f0e68c\"><b>"+testCaseMap.get(pair.getKey()).getName()+"</b></td>"+
			"<td align=\"left\" width=\"40%\" bgcolor=\"#f0e68c\"><b>"+testCaseMap.get(pair.getKey()).getTestID()+"</b></td><td align=\"center\" width=\"10%\" bgcolor=\"#deb887\"><b><font color=\""+strFonColor+"\"><a>"+testCaseMap.get(pair.getKey()).getStatus()+"</a></font></b></td></tr>";
					;
			
		}
		strSummaryFileContent=strSummaryFileContent+ "<tr><td colspan=\"4\" align=\"left\" width=\"100%\" bgcolor=\"#deb887\"><b>Execution Logs @ "+Config.getProp("UploadDirectory").replace("/", "\\")+"\\"+Config.getProp("runid")+"</b></td></tr>"
				+ "<tr><td colspan=\"4\" align=\"right\" width=\"100%\" bgcolor=\"#deb887\"><b><font color=\"green\"><a>PASSED</a></font>: "+intPassCounter+"<font color=\"red\"><a> FAILED</a></font>: "+intFailCounter+"</b></td></tr></table></body></html>";
		
		try (Writer writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("Summary.html"), "utf-8"))) 
		{
 ...
 **/
	private List<ITestSet> getTestSets(){
		List<ITestSet> testSets = new ArrayList<ITestSet>();
			...
			if (System.getProperty("gatewayUserID") == null)
				System.setProperty("gatewayUserID", "");
			if (System.getProperty("Authentication_Key") == null)
				System.setProperty("Authentication_Key", "");
			if (System.getProperty("gatewayEntityID") == null)
				System.setProperty("gatewayEntityID", "");
			/*
			...
			if (envName.contains("SANDBOX") || envName.contains("CERTIFICATION") || envName.contains("PRODUCTION")){				
				...
				if (accountEnabled && (System.getProperty("Authentication_Key").equals("")) || accountEnabled && (System.getProperty("gatewayEntityID").equals("")) || accountEnabled && (System.getProperty("gatewayUserID").equals(""))){
					System.out.println("Aborting test run... Please enter the Authentication_Key, gatewayEntityId and gatewayUserID and re-run");
					System.exit(1);
				}
			...
			Table resources = new Excel(Configurables.testsFilePath+Configurables.testFileName).querySheetRecords(temp_qry);
			...
			for (int i=0;i<dataSources.size();i++){
				...
				for (String testSuite : testSuiteSplit){
					...
					while (queryType<2){
						if (queryType==1){
							//query = "Select * from " + Utils.quote(dataSource) + " Where IncludeInRun='Y' and " +Utils.quote("tagName") + " = "+  Utils.sQuote(tagName) + "and "+Utils.quote("Test Suite ID")+" = " + Utils.sQuote(testSuite);
							query = "Select * from " + Utils.quote(dataSource) + " Where IncludeInRun='Y' and " +Utils.quote("tagName") + " like "+  Utils.sQuote("%"+tagName+"%") + "and "+Utils.quote("Test Suite ID")+" = " + Utils.sQuote(testSuite);
							
							
						}
						else if (queryType==2){
							query = "Select * from " + Utils.quote(dataSource) + " Where IncludeInRun='Y' and " + Utils.quote("tagName")+" = "+  Utils.sQuote("Wso2, "+tagName) +" and "+Utils.quote("Test Suite ID")+" = " + Utils.sQuote(testSuite);
							
							}
							else if (queryType==3){
								query = "Select * from " + Utils.quote(dataSource) + " Where IncludeInRun='Y' and " + Utils.quote("tagName")+" = "+  Utils.sQuote(tagName+", Sandbox") +  " and "+ Utils.quote("Test Suite ID")+" = " + Utils.sQuote(testSuite);
							}	
					
					System.out.println(Utils.getDateTime()+" Loading Test Suite data: "+query);
					Table testSheetData1 = new Excel(Configurables.testsFilePath+Configurables.testFileName).querySheetRecords(query);
					Configurables.testTableMap.put(dataSource, testSheetData1);
					//System.out.println(Utils.getDateTime()+" Query executed.");
					...
					//System.out.println(Utils.getDateTime()+" Adding to testSet...");
					 ...
					 */
					if(testSheetData1.recordCount()>0 && Config.getProp("enableAutoTCs").equalsIgnoreCase("yes"))
					{
						SwaggerData swagger = new SwaggerData();		
						swagger.generateTestCases(dataSource, testSuite, testSheetData1, resourceName, testSuiteName);	
				...
				//System.out.println(Utils.getDateTime()+" Added to testSet...");
	...
	private void addTestSetsToMap(List<ITestSet> testSets){
		for (ITestSet ts : testSets){
			...
			Collections.sort(list, new TestReportComparator());//Not used.
			
			//for (ITestInstance ti : ts.getInstances())
			for (ITestInstance ti : list)
			{
				...
				//testCaseMap.put(strTempKeyValue, ti);
				testCaseMap.put(ti.getID(), ti);
				//System.out.println("Debug "+ti.getName()+" Key: "+ti.getID());
	...
	public Table getTestCaseIDs(String datasheet, Table testSheetData1){
		...
		String tmp_query = "Select * from "+Utils.quote(datasheet)+" Where IncludeInRun='Y'";
		//System.out.println(Utils.getDateTime()+" Getting TC IDs...");
		
		
		//Table testIDs = new Excel(Configurables.testsFilePath+Configurables.testFileName).querySheetRecords(tmp_query);
		Table testIDs = testSheetData1;
		//System.out.println(Utils.getDateTime()+" Query executed for TC IDs: "+tmp_query);
		...
		if (records.toString().contains("TEST CASE ID")){
			...
			//System.out.println(Utils.getDateTime()+" TC IDs loaded...");
